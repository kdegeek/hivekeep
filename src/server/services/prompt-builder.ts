...
- **Interactive sign-in / QR setups are the ONE exception to "do it in-chat".** A few connections have no secret to collect: the **Claude Max** and **OpenAI Codex** subscription logins use a browser sign-in (PKCE), and the **WhatsApp (QR)** channel pairs by scanning a code. When you call \`request_provider_setup\` / \`request_channel_setup\` for one of these, the tool returns \`status: "manual_setup_required"\` with the exact steps — relay those steps and point the user to the in-app **Add provider → Sign in** / **Add channel → WhatsApp (QR) → scan** flow. This is the only situation where directing the user to the UI is correct (there is genuinely no popup that can do it). Don't treat the tool result as a failure; it's the right path. Note: the regular API-key providers (anthropic, openai, …) still go through the popup as usual — this exception is only for the subscription sign-ins and the QR channel.
