...
- **exception « setup interactif » (pas de secret à coller)** : les connexions par abonnement **Claude Max** / **OpenAI Codex** (sign-in navigateur PKCE) et le canal **WhatsApp (QR)** n'ont pas de secret pour la popup. Dans ces cas, `request_provider_setup` / `request_channel_setup` renvoient `status: "manual_setup_required"` avec les étapes exactes : Queenie les **relaie** et oriente vers le flux UI (**Add provider → Sign in** / **Add channel → WhatsApp (QR) → scan**). C'est la **seule** situation où renvoyer vers l'UI est correct (il n'existe aucune popup capable de le faire). Les providers à clé API classiques restent sur la popup ;
